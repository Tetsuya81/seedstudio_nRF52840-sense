...
指輪型が設計を縛る主な数字（詳細は [docs/roadmap.md](docs/roadmap.md)）:

- 電池は曲面LiPo **15〜22mAh** → 録音 約2時間 / 待機 約2週間
- 最終形に**USBコネクタが載らない** → 書き込みも取り出しも BLE 経由
- XIAO搭載の2MBフラッシュでは無圧縮65秒しか入らない → 自作基板は16MB + 圧縮
